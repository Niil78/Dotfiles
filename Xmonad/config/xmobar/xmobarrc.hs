...
Config {
       ...
       , sepChar          = "%"
       , alignSep         = "||"
       , template = "<action=`mygtkmenu .menurc` button=1><fn=5></fn></action>\
           <action=`xdotool key 0xffeb+0xff1b` button=3><action=`xdotool key 0xffe3+0xffe9+0xff51` button=4><action=`xdotool key 0xffe3+0xffe9+0xff53` button=5>%UnsafeStdinReader%</action></action></action>\
           <action=`mygtkmenu .placerc` button=1><action=`XMUptimeToggle` button=3><fn=5></fn></action></action>%UT%<action=`xdotool key 0xffeb+0xffe1+0x7a`>%MW%</action>|\
           <action=`XMDateToggle` button=1><action=`XMGSimplecal` button=3><action=`XMCal` button=45>%DO%<fn=1><fc=#ccc> %LT%</fc></fn></action></action></action>\
           <fc=#ccc><action=`XMGWeather` button=1>%WI%</action><fn=2> </fn><action=`XMAccuWeather` button=1><action=`XMOWM` button=3><fn=1>%CT%</fn></action></action></fc>|%UP%\
           <action=`XMNetToggle` button=1><action=`xdg-open http://192.168.0.1` button=2><action=`XMVnstat+h` button=3><action=`XMifconfig` button=45>%NT%%NS%</action></action></action></action>\
           <action=`XMTrafToggle` button=1><action=`XMTransgui` button=3><action=`XMVnstat-m` button=4><action=`XMVnstat-d` button=5>%TS%</action></action></action></action>%DT%\
           <action=`XMCPUToggle` button=1><fc=#ccc></fc></action>%CU%\
           <action=`XMTempToggle` button=1><action=`XMSensors` button=3><fc=#ccc></fc></action></action>%TA%\
           <action=`XMMemToggle` button=1><fc=#ccc></fc></action>%MU%%RU%\
           <action=`XMVolToggle` button=1><action=`XMPavucontrol` button=2><action=`XMMute` button=3><action=`pulseaudio-ctl down` button=4><action=`pulseaudio-ctl up` button=5>%VS%%VL%%PL%</action></action></action></action></action>%PS%\
           <action=`XMSoundCloud`>%SC%</action>\
           <action=`xkblayout-state set +1`>%kbd%</action>\
           <action=`XMToggleAll` button=1><action=`XMCleanAll` button=3><fc=#ccc>  </fc></action></action>"
